...
asm_grammar = Grammar(
    ...
    line                  = ws? (symbol_def / orphaned_bytes / orphaned_words / directive_use / instruction_use) comment? newline
    ...
    dot_include           = ws? ".include" ws string_literal
    dot_incchr            = ws? ".incchr" ws string_literal
    dot_org               = ws? ".org" ws symbol

    symbol_def            = macro_def / array_def / function_def / ram_var_def / local_var_def / label_def / const_def
    ...
    global_symbol         = ~"[A-Za-z][A-Za-z0-9_]*"
    ...
    expression            = (paren_group / number_literal / macro_parameter / function_call / symbol) (ws operator ws expression)?
    operator              = "&" / "<<" / ">>" / "+" / "-" / "*" / "|" / "/"
